	...
	*/
	...
	getItem: function(url){
		...
		if(hash.length==2){
			...
			this.quick_item = Lincko.storage.getClone(type, id);
			if(!this.quick_item){
				this.quick_item = {};
			}
	...
	model_action: {
		pitch: function(item){
			...
			var app_generic_state_action = function(){
				if(Lincko.storage.get("pitch", item.id)){
					app_content_menu.selection("question", item.id);
				}
			}
			if(!storage_first_launch && Lincko.storage.get("pitch", item.id)){
		...
		question: function(item){
			var item = item;
			var app_generic_state_action = function(){
				if(Lincko.storage.get("question", item.id)){
					app_content_menu.selection("answer", item.id);
				}
			}
			if(!storage_first_launch && Lincko.storage.get("question", item.id)){
				app_generic_state_action();
			} else {
				Lincko.storage.getLatest(false, app_generic_state_action);
			}
		...
		answer: function(item){
			var item = item;
			var app_generic_state_action = function(){
				var question = Lincko.storage.getParent("answer", item.id);
				if(question){
					app_content_menu.selection("answer", question.id);
				}
			}
			if(!storage_first_launch && Lincko.storage.get("answer", item.id)){
				app_generic_state_action();
			} else {
				Lincko.storage.getLatest(false, app_generic_state_action);
			}
		...
		file: function(item){
			...
			if(!storage_first_launch && Lincko.storage.get("file", item.id)){
				app_generic_state_action();
			} else {
				Lincko.storage.getLatest(false, app_generic_state_action);
			}
