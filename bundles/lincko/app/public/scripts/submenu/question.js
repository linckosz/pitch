submenu_list['app_question_new'] = {
	...
	"_title": {
		"style": "title",
		"title": Lincko.Translation.get('app', 2104, 'html'), //New Question
	},
	"question_title": {
		"style": "input_text",
		...
		"value": function(Elem, subm){
			var items = Lincko.storage.list('question', -1, null, 'pitch', subm.param);
			var param = {
				number: items.length+1,
			};
			return Lincko.Translation.get('app', 2106, 'html', param); //Question #[{number]}
		},
	...
	"create": {
		...
		"action": function(Elem, subm){
			...
			var parent = Lincko.storage.get("pitch", subm.param);
...
submenu_list['app_question_edit'] = {
	...
	"_title": {
		"style": "title",
		"title": Lincko.Translation.get('app', 2105, 'html'), //Edit Question
	},
	"question_title": {
		"style": "input_text",
		"title": Lincko.Translation.get('app', 28, 'html'), //Title
		"name": "title",
		"value": function(Elem, subm){
			return Lincko.storage.get('question', subm.param, 'title');
		},
		"class": "submenu_input_text",
		"now": function(Elem, subm){
			Elem.on('keypress', subm, function(event) {
	...
	"deletion": {
		...
		"action": function(Elem, subm){
			if(confirm(Lincko.Translation.get('app', 26, 'js'))){ //Are you sure you want to delete this item?
				...
				var item = Lincko.storage.get('question', subm.param);
