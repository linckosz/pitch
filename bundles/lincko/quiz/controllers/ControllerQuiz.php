...
class ControllerQuiz extends Controller {
	...
	public function question_get($sessionid_enc){
		$app = ModelLincko::getApp();
		$app->lincko->data['data_sessionid_enc'] = $sessionid_enc;
		$app->lincko->data['get_refresh'] = false;
		$sessionid = STR::integer_map($sessionid_enc, true);
		if($session = Session::Where('id', $sessionid)->first(array('id', 'statistics_id'))){
			if($statistics = Statistics::Where('id', $session->statistics_id)->first(array('id', 'question_id'))){
	...
	public function answer_get($sessionid_enc, $answerid_enc){
		$app = ModelLincko::getApp();
		...
		if($session = Session::Where('id', $session_id)->whereNotNull('statistics_id')->where('updated_ms', '>', $limit_ms)->first(array('id', 'pitch_id', 'statistics_id'))){
			$answer = Answer::Where('id', $answer_id)->first(array('id', 'number', 'parent_id'));
			//Check if already answered
			if(false && Answered::Where('guest_id', $guest_id)->where('session_id', $session->id)->where('question_id', $answer->parent_id)->first(array('id'))){ //toto
				$app->lincko->data['data_answered'] = true;
				$app->render('/bundles/lincko/quiz/templates/quiz/result/wait.twig');
				return true;
			} else if($answer && $statistics = Statistics::Where('id', $session->statistics_id)->where('session_id', $session->id)->first()){
				...
				if(isset($statistics->$letter)){
					if($question = Question::Where('id', $answer->parent_id)->first(array('id', 'answer_id'))){
						...
						$answered->session_id = $session->id;
