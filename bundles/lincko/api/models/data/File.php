...
class File extends ModelLincko {
	...
	protected static $parent_list = array('pitch', 'question', 'answer');
...
////////////////////////////////////////////
	...
	public function pitch(){
		return $this->belongsTo('\\bundles\\lincko\\api\\models\\data\\Pitch', 'id', 'file_id');
	}

	//One(File) to One(Question)
	public function question(){
		return $this->belongsTo('\\bundles\\lincko\\api\\models\\data\\Question', 'id', 'file_id');
	}

	//One(File) to One(Question)
	public function answer(){
		return $this->belongsTo('\\bundles\\lincko\\api\\models\\data\\Answer', 'id', 'file_id');
	}
...
////////////////////////////////////////////
	...
	public static function setItem($form){
		...
		if($new){
			$error = true;
			if(isset($form->parent_type) && isset($form->parent_id) && isset($form->parent_md5)){
				if(is_string($form->parent_type) && is_numeric($form->parent_id) && is_string($form->parent_md5) && in_array($form->parent_type, static::$parent_list)){
		...
		failed:
		if($new){
			$errmsg = $app->trans->getBRUT('data', 11, 1)."\n"; //File upload failed
		} else {
...
////////////////////////////////////////////
	...
	public function save(array $options = array()){
		...
		if(!$this->id){ //Only copy a file for new items
			...
			try {
				...
				$folder_ori->createPath($app->lincko->filePath.'/'.$app->lincko->data['uid'].'/');
				$this->thu_type = null;
				$this->thu_ext = null;
				$this->progress = 100;
				...
				if($this->category=='image'){
					...
					$folder_thu->createPath($app->lincko->filePath.'/'.$app->lincko->data['uid'].'/thumbnail/');
					try {
						$src = WideImage::load($this->tmp_name);
						...
						$this->thu_type = 'image/png';
						$this->thu_ext = 'png';
						copy($app->lincko->path.'/bundles/lincko/api/public/images/generic/unavailable.png', $folder_thu->getPath().$this->link.'.png');
					}
					...
					$folder_txt->createPath($app->lincko->filePath.'/'.$app->lincko->data['uid'].'/convert/'); //Because of exec limitation (does not work with ssh2.sftp), we use local link
					$this->thu_type = 'image/jpeg';
					$this->thu_ext = 'jpg';
					...
					$destination = $app->lincko->filePath.'/'.$app->lincko->data['uid'].'/'.$this->link.'.'.$this->ori_ext;
					$thumbnail = $app->lincko->filePath.'/'.$app->lincko->data['uid'].'/thumbnail/'.$this->link.'.'.$this->thu_ext;
	...
	public function checkProgress(){
		if($this->category=='video' && $this->progress<100 && !$this->error && isset($this->id)){
			$app = ModelLincko::getApp();
			$path = $app->lincko->filePath.'/'.$this->uploaded_by.'/convert/'.$this->link;
	...
	public function setProgress(){
		if($this->category == 'video' && $this->progress < 100 && !$this->error){
			...
			$path = $app->lincko->filePath.'/'.$this->uploaded_by.'/convert/'.$this->link;
			$file = $app->lincko->filePath.'/'.$this->uploaded_by.'/'.$this->link.'.'.$this->ori_ext;
			$loop = true;
			$progress = 100;
			$try = 10; //5s of try
			$time = $this->freshTimestamp();
			$time_ms = \micro_seconds();
			self::where('id', $this->id)->getQuery()->update(['progress' => 1, 'updated_at' => $time, 'updated_ms' => $time_ms]);
			...
			while($loop){
				...
				$time = $this->freshTimestamp();
				$time_ms = \micro_seconds();
				self::where('id', $this->id)->getQuery()->update(['progress' => $progress, 'size' => $size, 'updated_at' => $time, 'updated_ms' => $time_ms]);
				usleep(rand(10000, 15000)); //10ms
			}
			if($try<=0){
				$time = $this->freshTimestamp();
				$time_ms = \micro_seconds();
				self::where('id', $this->id)->getQuery()->update(['progress' => 100, 'size' => $size, 'error' => 1, 'updated_at' => $time, 'updated_ms' => $time_ms]);
				usleep(rand(10000, 15000)); //10ms
			}
