...
////////////////////////////////////////////
	...
	protected static function errorMsg($detail='', $msg=false, $resignin=false){
		$app = ModelLincko::getApp();
		if(!$msg){
			$msg = $app->trans->getBRUT('data', 0, 5); //You are not allowed to edit the server data.
		}
	...
	public function save(array $options = array()){
		...
		$updated_ms = \micro_seconds();
		$this->updated_ms = $updated_ms;
	...
	public function touchUpdateAt($users_tables=array()){
		$app = ModelLincko::getApp();
		if (!$this->timestamps || !isset($this->updated_at) || !isset($this->updated_ms) || !isset($this->id)) {
			return false;
		}
		$time = $this->freshTimestamp();
		$time_ms = \micro_seconds();
		$result = $this::withTrashed()->where('id', $this->id)->getQuery()->update(['updated_at' => $time, 'updated_ms' => $time_ms]);
		usleep(rand(10000, 15000)); //10ms
		$this->forceRead();
		return $users_tables;
	}
	...
	public function getNoSQL($force=false){
		...
		if($force || !isset($this->nosql) || is_null($this->nosql)){
			...
			//Must always include created_at
			if(!isset($result->created_at)){ $result->created_at = (int) (new \DateTime($result->created_at))->getTimestamp(); }
			//Must always include updated_ms
			if(!isset($result->updated_ms)){ $result->updated_ms = (int) $this->updated_ms; }
