...
class ControllerData extends Controller {
	...
	public function latest_post(){
		...
		foreach (self::$priority_tables as $table) {
			...
			$list[$table] = array();
			$class = ModelLincko::getClass($table);
			if($class){
		...
		if($lastvisit_ms && isset($result->read)){
			foreach ($result->read as $table => $items) {
				foreach ($items as $id => $item) {
					if(isset($item->updated_ms) && $item->updated_ms <= $lastvisit_ms){
		...
		if($lastvisit_ms){
			...
			$msg = array('data' => $result, 'info' => 'all');
		}
		(new Json($msg))->render();
		return exit(0);
	}
	...
	protected function additional_data(&$result){
		$force_read = ModelLincko::getForceRead();
