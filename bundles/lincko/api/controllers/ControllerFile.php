...
<html>
	...
	public function upload_post(){
		...
		$failmsg = $app->trans->getBRUT('data', 11, 1)."\n"; //File upload failed.
		$errmsg = $failmsg.$app->trans->getBRUT('data', 0, 7); //Please try again.
		$errfield = 'undefined';
		...
		if(isset($_FILES)){
			...
			foreach ($files_array as $key => $value) {
				...
				if(is_object($model)){
					$parent = false;
					if($parent_class = ModelLincko::getClass($data->parent_type)){
					...
					$model->uploaded_by = $app->lincko->data['uid'];
					...
					if($model->save()){
						//$parent
						if(array_key_exists('file_id', $parent->getAttributes())){
							$parent->file_id = $model->id;
							if($parent->save()){
			...
			$errmsg = $failmsg.$app->trans->getBRUT('data', 3, 2); //No file selected to upload.
		}

		if($success){
			$msg = array('data' => $result, 'msg' => $app->trans->getBRUT('data', 11, 2)); //File uploaded.
			(new Json($msg))->render();
			return exit(0);
		} else {
			if(isset($_FILES)){
	...
	public function qrcode_get(){
		...
		$folder = new Folders;
		$folder->createPath($app->lincko->filePath.'/'.$app->lincko->data['uid'].'/qrcode/');
	...
	public function open_get($type, $md5, $id){
		...
		if($file = File::Where('id', $id)->where('md5', $md5)->first()){
			...
			$name = $file->name;
			if($type=='download'){
				$path = $app->lincko->filePath.'/'.$file->uploaded_by.'/'.$file->link.'.'.$file->ori_ext;
				...
				if($type=='thumbnail'){
					$path = $app->lincko->filePath.'/'.$file->uploaded_by.'/thumbnail/'.$file->link.'.'.$file->thu_ext;
